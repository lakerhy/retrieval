% Not write to the file, passing the variables to instead

function [Tb]=icemain_new(ffile,tfile,par,type)
%   MEMLS
...
%         = slred(num,roi,epsi,epsii,tei,sih,siv,di,dei,Ti,pci,freq,Wi,gai,si,sal);
    ...
    %[rnum,rroi,repsi,repsii,rtei,rsih,rsiv,rdi,rdei,rTi,rpci,rWi,rgai,rtype,rsal]
    %=
    %slred(num,roi,epsi,epsii,tei,sih,siv,di,dei,Ti,pci,freq,Wi,gai,type,sal);itemizeitemizeitemize 
    ...
    %% rtype = type;
       rfy=(rtype==3);
       rmy=(rtype==4);
    [gbih,gbiv,gs6,ga2i] =    scice(rtype,gbih,gbiv,gs6,ga2i,rTi,rsal,freq,rpci);
