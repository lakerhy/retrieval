...
%*******************************
function[T_B,E_eff]=fw_fun2(par,ice_type,V)
%function[T_B]=fw_fun2(T_s,V,di_snow,roi_snow,pci_snow,di_ice,sal, ...
%                      ice_type)
...
% %    discretize('input_MY',snow_layernums,'syntes.disc.MY',1);
    ...
    Tb_MY = icemain('Freq-Memls-in.txt','Angl-Memls-in.txt', par,4);
% Using the
% emissivity_MY =Tb_MY/T_s

% Using the 
 emissivity_MY = emi(par,4)
  ...
  if ice_type==2 %FY
    ...
    Tb_FY = icemain('Freq-Memls-in.txt','Angl-Memls-in.txt',par,3);
    
    
%    emissivity_MY =Tb_MY/T_s;
    
       emissivity_FY = emi(par,3)
       
  E_eff_H=emissivity_FY(1:5,2);
  E_eff_V=emissivity_FY(1:5,1);
   T_BH_MEMLS = Tb_FY(:,2);
  T_BV_MEMLS= Tb_FY(:,1);
 
  TbFY=[T_BV_MEMLS(1);T_BH_MEMLS(1);T_BV_MEMLS(2);T_BH_MEMLS(2);T_BV_MEMLS(3);T_BH_MEMLS(3);T_BV_MEMLS(4);T_BH_MEMLS(4);T_BV_MEMLS(5);T_BH_MEMLS(5)];
  end
...
%   T_BS_V= Tb_MY(1:5,1);
  ...
  T_BH=(T_BU+(tau.*(T_BH_MEMLS+T_BOmegaH)))';     %Equation (10); Horizontal pol.
  T_BV=(T_BU+(tau.*(T_BV_MEMLS+T_BOmegaV)))';     %Equation (10); Vertical pol.


  T_B=[T_BV(1); T_BH(1); T_BV(2); T_BH(2); T_BV(3); T_BH(3); T_BV(4); T_BH(4); ...
       T_BV(5); T_BH(5)]';

E_eff=[E_eff_V(1);E_eff_H(1);E_eff_V(2);E_eff_H(2);E_eff_V(3);E_eff_H(3);E_eff_V(4);E_eff_H(4);E_eff_V(5);E_eff_H(5)]';
